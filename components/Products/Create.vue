  ...
  <Card>
    <template #content>
      <div class="flex items-center gap-4 w-full mb-2">
        ...
        <h1 class="text-surface-900 dark:text-surface-0 text-xl font-semibold">Create Product</h1>
      </div>
      <Form 
        v-slot="$form" 
      ...
      >
        ...
        <div class="flex justify-end gap-2 mt-4">
          <Button type="submit" label="Create" severity="success"  icon="pi pi-check" />
        </div>
      </Form>
    </template>
  </Card>
...
const onFormSubmit = async({ valid, states }) => {
  if (valid) {
    ...
    const result = await productsStore.createProduct(states);
    if(result.success) {
